...
our $p = Nagios::Plugin->new(
  ...
  [ -t <timeout> ] [ -r <retries> ] [ -g|--getinfos ]
  [ -v|--verbose ] ",
        version => $VERSION,
...
for (my $i=0;$i<=$retries;$i++) {
  ...
  unless ( defined($inputfile) ) {
    ...
    $client = new IO::Socket::SSL->new(PeerAddr => $host, SSL_verify_mode => SSL_VERIFY_NONE, $sslv3 ? 
    ( SSL_version => 'SSLv3' ) : () );
    unless ( $client ) {
      $p->nagios_exit(
        return_code => "UNKNOWN",
...
if ( $getinfos ) {
  ...
  $message = "($product[1] - SystemROM: $xml->{'FIRMWARE_INFORMATION'}[0]->{'INDEX_1'}[0]->{'FIRMWARE_VERSION'}[0]->{'VALUE'} -$serial_number $xml->{'FIRMWARE_INFORMATION'}[0]->{'INDEX_4'}[0]->{'FIRMWARE_NAME'}[0]->{'VALUE'} FW $xml->{'FIRMWARE_INFORMATION'}[0]->{'INDEX_4'}[0]->{'FIRMWARE_VERSION'}[0]->{'VALUE'}" . "$server_name) " if defined $product[1];
}
